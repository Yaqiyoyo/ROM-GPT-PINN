...
def gpt_train(GPT_PINN, mu, xyz_resid, xyz_BC, curl2_term, BC_values,
             epochs_gpt, lr_gpt, path, largest_loss=None,
              largest_case=None, testing=False):
    
    GD = grad_descent(mu, xyz_resid, xyz_BC, curl2_term, BC_values,lr_gpt)



    if (testing == False):
        print(f"gpt training step: mu equal to {mu}")
        logger.info(f"gpt training step: mu equal to {mu}")
        loss_values_list = []
        ...
        # plt.plot(range(epochs_gpt + 1), loss_values_list,  label='train loss')
        # plt.legend(loc = 'best')
        # plt.xlabel('epochs')
        # plt.ylabel('GPT_PINN loss')
        # plt.savefig(fr"{path}/loss.png", format='png')
        # plt.title(fr"$\mu = {mu}$", fontsize=20)
        # plt.show()
